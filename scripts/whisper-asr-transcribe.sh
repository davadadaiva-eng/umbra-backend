#!/usr/bin/env bash
# Wait for the Whisper-ASR server to become ready, then transcribe audio.
#
#   npm run whisper:asr-transcribe -- /path/to/audio.wav
#
# The server answers /health immediately and flips to `ready` once the ~520 MB
# models load; this blocks until then and prints the speaker-labeled segments.
set -e
...
exec python "$SCRIPT_DIR/whisper_asr_transcribe.py" "$@"
