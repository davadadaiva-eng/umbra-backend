#!/usr/bin/env python
"""
Wait for the VibeVoice-ASR server to finish loading, then transcribe an audio
file and print the speaker-labeled segments.

Usage (from the VibeVoice venv):

    cd external/VibeVoice
    ./.venv/Scripts/python.exe ../../scripts/vibevoice_asr_transcribe.py \
        /tmp/meeting-test/meeting.wav

The server (npm run vibevoice:asr-server) serves /health immediately and flips
from `state: loading` to `ready` once the model has finished its (potentially
multi-hour, ~17 GB) first download. This script polls /health until then,
then POSTs the file and pretty-prints the diarized result.
"""
import argparse
import json
import sys
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Wait for VibeVoice-ASR then transcribe audio")
    parser.add_argument("audio", nargs="?", default=DEFAULT_AUDIO)
    parser.add_argument("--url", default="http://127.0.0.1:17500")
    parser.add_argument("--context", default="")
    parser.add_argument("--poll", type=int, default=10, help="seconds between /health polls")
